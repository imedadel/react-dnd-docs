...
Now that you've read [the overview](./overview), it's adventure time!
...
We will use this example to demonstrate the data-driven philosophy of `react-dnd`. You will learn how to create a _drag source_ and a _drop target_, wire them together with your React components, and change their appearance in response to the drag and drop events.
...
- `collect` defines a _collector function_: this is basically a way to transform state from the drag-and-drop system into usable props for your components.
- The `result array` contains
  ...
  - A _ref function_ as the second item. This is used to attach your DOM elements to react-dnd.
